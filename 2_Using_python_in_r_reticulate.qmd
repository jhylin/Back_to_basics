...
```{r}
# Transfer above short code into a function and saved as a .py script
...
# Sample plot for trial only 
# (will need to use a different dataset to show a more meaningful plot actually... plot looks funny at the moment!)
ggplot(df, aes(Max_Phase, QED)) + geom_point()
```