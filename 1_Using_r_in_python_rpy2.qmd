...
Essentially, I think there are possibly two packages that may do the trick - one is rpy2 and the other is reticulate. So I'll try rpy2 first in this post.
...
The first coding issue encountered is that the code to produce a final output of the dataframe won't transform into a .qmd preview i.e. html page. There is an error message showing such as, "Conversion rules for `rpy2.robjects` appear to be missing..." when trying to preview this as a .qmd. One of the solutions I've found online is to set up a local converter in the code and run all rpy2-related code within this converter (as commented in the code above).

The second issue is that I've only managed to run the code to show R dataframe in Python but not the plot... hence you will see the error message generated from the code above, with only the dataframe column names shown without a plot there. To resolve this issue, more time is spent on looking for answers online. A mini test was eventually done in Jupyter notebook to show a short code to achieve a simple plot (**link to .ipynb in repo to be added**) by calling R magic in Python kernel via rpy2. The full code to import dataframe and make a simple scatter plot based on the data in the dataframe is shown below.
